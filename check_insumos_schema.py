...
def check_insumos_schema():
    url = f"{SUPABASE_URL}/rest/v1/insumos?select=*&limit=1"
    ...
    try:
        with urllib.request.urlopen(req) as response:
            ...
            if len(data) > 0:
                print("Columns in 'insumos':")
                for key in data[0].keys():
                    print(f"- {key}")
            else:
                print("No ingredients found to check schema.")
    except Exception as e:
        print(f"Error: {e}")

if __name__ == "__main__":
    check_insumos_schema()
